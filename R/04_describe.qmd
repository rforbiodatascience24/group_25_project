...
RPKMplot <- filtered_data |>
  filter(experiment %in% c("RPKM_control", "RPKM_tau")) |> 
  ggplot(mapping = aes(x = gene_expression, fill = experiment)) + 
  geom_density(alpha = 0.5) + 
  theme_minimal() + 
  labs(title = "RPKM Data", y = "Density", x = "Mean Gene Expression")

RPMplot <- filtered_data |>
  filter(experiment %in% c("RPM_control", "RPM_tau")) |> 
  ggplot(mapping = aes(x = gene_expression, fill = experiment)) + 
  geom_density(alpha = 0.5) + 
  theme_minimal() + 
...
readsplot <- filtered_data |>
  filter(experiment %in% c("reads_control", "reads_tau")) |> 
  ggplot(mapping = aes(x = gene_expression, fill = experiment)) + 
  geom_density(alpha = 0.5) + 
  theme_minimal() + 
