...
However, this is not the case. Seems like there are 21 that have data for more than 18 experiments.
...
filtered_data <- describe_input_long |>
  filter(!gene_ID %in% to_be_deleted)
```

Now we have 20023 genes left
...
filtered_data_for_analysis <- filtered_data |> 
  mutate(base_experiment = sub("_\\d+$", "", experiment)) |>
  group_by(gene_ID, base_experiment) |>
  summarize(mean_reads = mean(mean_reads),
            mean_rpm = mean(mean_rpm),
            mean_rpkm = mean(mean_rpkm),
            gene_expression_mean = mean(gene_expression),
            .groups = "drop")
```

The data has now 120138 observations in total.
...
We would like to visualize the filtered data, to gain an overview of the values we are about to analyze next.

Will: try either density plot or barplot of the gene expressions. If looks bad then maybe scatter plot? If it looks like there is just too much data, try maybe filter for top and bottom 1000 obsercations?
