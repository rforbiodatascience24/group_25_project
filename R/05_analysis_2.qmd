...
ggplot(filtered_genes, aes(x = reorder(gene_symbol, fold_change_rpkm), y = fold_change_rpkm)) +
...
Now, we will do the GSEA using the MSigDB. We will also plot the 8 more under and over expressed gene sets.

```{r}
  # Step 3: Retrieve gene sets from MSigDB (Hallmark pathways as an example)
msigdb_gene_sets <- msigdbr(species = "Homo sapiens", category = "H") |>
  select(gs_name, gene_symbol)

# Step 4: Perform Gene Set Enrichment Analysis
...
# Step 5: Filter and visualize the top 10 and bottom 10 enriched gene sets
top_and_bottom_gsea <- gsea_analysis |>
  arrange(desc(enrichment_score)) |>
  slice(c(1:8, (n() - 7):n()))|>
  mutate(gs_name = str_remove(gs_name, "HALLMARK_"))  # Remove "HALLMARK_" prefix

ggplot(top_and_bottom_gsea, aes(x = reorder(gs_name, enrichment_score), y = enrichment_score)) +
  geom_col(aes(fill = enrichment_score > 0)) +  # Color based on positive/negative enrichment
  coord_flip() +
  labs(
